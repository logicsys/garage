	...
	async fn handle(
		...
		let resp = match endpoint {
			Endpoint::HeadObject {
				...
				part_number,
				version_id,
			} => handle_head(ctx, &req.map(|_| ()), &key, part_number, version_id).await,
			Endpoint::GetObject {
				key,
				part_number,
				version_id,
				...
				handle_get(ctx, &req.map(|_| ()), &key, part_number, overrides, version_id).await
			}
			Endpoint::UploadPart {
				key,
				part_number,
			...
			Endpoint::DeleteObject { key, version_id } => {
				handle_delete(ctx, &key, version_id).await
			}
			...
			Endpoint::ListObjectVersions {
				...
				handle_list_object_versions(ctx, &query).await
			}
			...
			Endpoint::GetObjectRetention { key, .. } => {
				handle_get_object_retention(ctx, &key).await
			}
			Endpoint::PutObjectRetention { key, .. } => {
				handle_put_object_retention(ctx, req, &key).await
			}
			Endpoint::GetObjectLegalHold { key, .. } => {
				handle_get_object_legal_hold(ctx, &key).await
			}
			Endpoint::PutObjectLegalHold { key, .. } => {
				handle_put_object_legal_hold(ctx, req, &key).await
			}
			endpoint => Err(Error::NotImplemented(endpoint.name().to_owned())),
		};
